...
**このアプリと Receiver の通信は暗号化されていません。**
`ws://` は平文なので、同じ Wi‑Fi にいる第三者に通信の中身を読み取られる可能性があります。

想定している使い方は **「自分の家の Wi‑Fi で、自分の PC を操作する」** です。
この前提が崩れる場所では使わないでください。

| | |
| --- | --- |
| **使ってよい** | 自宅など、繋いでいる人を自分で把握できる Wi‑Fi |
| **使わないほうがよい** | 社内・学校・カフェ・ホテル・空港・寮などの共有 Wi‑Fi |
| **送ってはいけないもの** | パスワード、クレジットカード番号、その他の秘密（「文字入力 → 送信」はそのまま平文で流れます） |

具体的なリスクは次の 2 点です。

- **合言葉ごと傍受されうる。** 合言葉は 32 桁の 16 進数（128 bit）なので推測は不可能ですが、
  平文で流れるため傍受はされます。WPA2 パーソナルのように接続パスワードを共有する Wi‑Fi では、
  同じネットワークの参加者が通信を復号できます。
- **合言葉を得た相手は PC を完全に操作できる。** マウス・キーボード・クリップボードが自由に使えるため、
  実質的に PC の前に座っているのと同じことができます。

合言葉は Receiver を起動するたびに新しくなり、古いものは無効になります。
**使い終わったら Receiver を終了してください。**それが最も確実な遮断方法です。
また、QR コードには合言葉が含まれます。画面共有や配信の最中に表示したままにしないでください。

> `wss://`（暗号化あり）に対応していないのは、Receiver が自己署名証明書を使うことになり、
> ブラウザの証明書警告という別の壁が立つためです。この制約は既知の課題として残しています。
